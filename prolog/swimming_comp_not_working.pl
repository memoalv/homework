...
constraints( Team ) :-
		...
		% neg
		% 1
		notMember(camp(sam, brown, woodlandPark, _, _), Team),
		notMember(camp(sam, brown, _, purple, _), Team),
		notMember(camp(sam, brown, _, purple, _), Team),
		notMember(camp(dan, _, forestRidge, _, _), Team),
		% % 3
		notMember(camp(mark, manor, _, _, _), Team),
		notMember(camp(mark, smart, _, _, _), Team),
		notMember(camp(_, _, lakeside, _, divingComp), Team),
		% % 4
		notMember(camp(_, _, forestRidge, red, _), Team),
		notMember(camp(_, _, forestRidge, blue, _), Team),
		notMember(camp(bill, _, _, _, freestyleRace), Team).
